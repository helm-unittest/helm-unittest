...
| `equal`                               | **path**: *string*. The `set` path to assert.<br/>**value**: *any*. The expected value.<br/>**decodeBase64**: *bool, optional*. Decode the base64 before checking<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                                | Assert the value of specified **path** equal to the **value**.                                                                                                                                                                   | <pre>equal:<br/>  path: metadata.name<br/>  value: my-deploy</pre>                                                                                                                                                                                       |
| `notEqual`                            | **path**: *string*. The `set` path to assert.<br/>**value**: *any*. The value expected not to be. <br/>**decodeBase64**: *bool, optional*. Decode the base64 before checking<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                     | Assert the value of specified **path** NOT equal to the **value**.                                                                                                                                                               | <pre>notEqual:<br/>  path: metadata.name<br/>  value: my-deploy</pre>                                                                                                                                                                                    |
...
| `exists`<br/>(deprecates `isNotNull`) | **path**: *string*. The `set` path to assert.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                                                                    | Assert if the specified **path** `exists`.                                                                                                                                                                                       | <pre>exists:<br/>  path: spec.strategy</pre>                                                                                                                                                                                                             |
| `notExists`<br/>(deprecates `isNull`) | **path**: *string*. The `set` path to assert.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                                                                    | Assert if the specified **path** NOT `exists`.                                                                                                                                                                                   | <pre>notExists:<br/>  path: spec.strategy</pre>                                                                                                                                                                                                          |
...
| `greaterOrEqual`                      | **path**: *string*. The `set` path to assert.<br/>**value**: *int, float, string*.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                               | Assert the value of specified **path** is greater or equal to the **value**.                                                                                                                                                     | <pre>greaterOrEqual:<br/>  path: resources.requests.cpu<br/>  value: 2</pre>                                                                                                                                                                             |
| `notGreaterOrEqual`                   | **path**: *string*. The `set` path to assert.<br/>**value**: *int, float, string*.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                               | Assert the value of specified **path** is NOT greater or equal to the **value**.                                                                                                                                                 | <pre>notGreaterOrEqual:<br/>  path: resources.requests.cpu<br/>  value: 2</pre>                                                                                                                                                                          |
...
| `lessOrEqual`                         | **path**: *string*. The `set` path to assert.<br/>**value**: *int, float, string*.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                               | Assert the value of specified **path** is less or equal to the **value**.                                                                                                                                                        | <pre>lessOrEqual:<br/>  path: spec.runAsUser<br/>  value: 2000</pre>                                                                                                                                                                                     |
| `notLessOrEqual`                      | **path**: *string*. The `set` path to assert.<br/>**value**: *int, float, string*.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                               | Assert the value of specified **path** is NOT less or equal to the **value**.                                                                                                                                                    | <pre>notLessOrEqual:<br/>  path: spec.runAsUser<br/>  value: 2000</pre>                                                                                                                                                                                  |
...
| `isNullOrEmpty`<br/>*`isEmpty`*       | **path**: *string*. The `set` path to assert.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                                                                    | Assert the value of specified **path** is null or empty (`null`, `""`, `0`, `[]`, `{}`).                                                                                                                                         | <pre>isNullOrEmpty:<br/>  path: spec.tls</pre>                                                                                                                                                                                                           |
| `isNotNullOrEmpty`<br/>*`isNotEmpty`* | **path**: *string*. The `set` path to assert.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                                                                    | Assert the value of specified **path** is NOT null or empty (`null`, `""`, `0`, `[]`, `{}`).                                                                                                                                     | <pre>isNotNullOrEmpty:<br/>  path: spec.selector</pre>                                                                                                                                                                                                   |
| `isSubset`                            | **path**: *string*. The `set` path to assert, the value must be an *object*. <br/>**content**: *any*. The content to be contained.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                                                               | Assert the object as the value of specified **path** that contains the **content**.                                                                                                                                              | <pre>isSubset:<br/>  path: spec.template<br/>  content:<br/>    metadata: <br/>      labels: <br/>        app: basic<br/>        release: MY-RELEASE<br/></pre>                                                                                          |
| `isNotSubset`                         | **path**: *string*. The `set` path to assert, the value must be an *object*. <br/>**content**: *any*. The content NOT to be contained.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                                                                           | Assert the object as the value of specified **path** that NOT contains the **content**.                                                                                                                                          | <pre>isNotSubset:<br/>  path: spec.template<br/>  content:<br/>    metadata: <br/>      labels: <br/>        app: basic<br/>        release: MY-RELEASE<br/></pre>                                                                                       |
| `isType`                              | **path**: *string*. The `set` path to assert.<br/>**type**: *string*. The expected type of the value.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                            | Assert the **type** of the object is equal to the value of the specified **path**                                                                                                                                                | <pre>isType:<br/>  path: metadata.name<br/>  type: string</pre>                                                                                                                                                                                          |
| `isNotType`                           | **path**: *string*. The `set` path to assert.<br/>**type**: *string*. The expected type of the value.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                                                                                                            | Assert the **type** of the object is NOT equal to the value of the specified **path**                                                                                                                                            | <pre>isNotType:<br/>  path: metadata.name<br/>  type: string</pre>                                                                                                                                                                                       |
| `lengthEqual`                         | **path**: *string, optional*. The `set` path to assert the count of array values. <br/>**paths**: *string, optional*. The `set` array of paths to assert the count validation of the founded arrays. <br/>**count**: *int, optional*. The count of the values in the array.<br/>**parse**: *string, optional*. Parse the string value at **path** as `json` or `yaml` before asserting.<br/>**innerPath**: *string, optional*. Path within the parsed content, same syntax as **path**. Requires **parse**.                                                      | Assert the **count** of the **path** or **paths** to be equal.                                                                                                                                                                   | <pre>lengthEqual:<br/>  path: spec.tls<br/>  count: 1<br/></pre>                                                                                                                                                                                         |
...
### Asserting on Embedded JSON or YAML

Charts often emit serialized content into a string field — a ConfigMap holding a
`config.json`, a Grafana dashboard, or a base64-encoded Secret. Asserting on the
raw string is fragile, because it depends on key ordering and whitespace that are
not guaranteed stable.

The `parse` field parses the string before the assertion runs, so the comparison
...
`parse` accepts `json` or `yaml`.

**Drilling into parsed content with `innerPath`**

`innerPath` selects a value inside the parsed content, using the same syntax as
`path`:
...
Arrays work too, including at the root of the parsed document:

```yaml
- equal:
    path: data["dashboard.json"]
    parse: json
    innerPath: panels[0].title
    value: Latency
```

When `innerPath` matches several values, every match must satisfy the assertion —
the same behaviour as a multi-match `path`. An `innerPath` that matches nothing is
reported as an unknown path, naming both the outer path and the inner one.

**Partial comparison**

Combine `parse` with `isSubset` to assert on some keys without restating the
whole document:
...
Note that `isSubset` matches the top-level keys of `content`; nested values are
compared in full. To assert a single nested value, use `innerPath`.

**Combining with `decodeBase64`**

Decoding happens before parsing, so a Secret containing base64-encoded JSON
works:
...
Numbers are typed by value, matching how helm-unittest already reads ordinary
paths: an integral value is an integer, and a non-integral value is a float. So
`8080`, `1.0` and `1e3` are all integers, while `1.5` is a float. A value written
as `8080` in the test file therefore matches a parsed JSON `8080`.

This makes type assertions on parsed content meaningful. A hand-written template
that interpolates a value without quotes emits a JSON number even when the input
was a string, which `isType` can catch:

```yaml
# chart:  "port": {{ .Values.portStr }}   renders  "port": 8080
- isType:
    path: data["config.json"]
    parse: json
    innerPath: port
    type: string   # fails: the template dropped the quotes
```

**Supported assertions**

`parse` and `innerPath` are available on `equal`, `notEqual`, `isSubset`,
`isNotSubset`, `contains`, `notContains`, `exists`, `notExists`, `isNullOrEmpty`,
`isNotNullOrEmpty`, `isType`, `isNotType`, `lengthEqual`, `notLengthEqual`,
`greaterOrEqual`, `notGreaterOrEqual`, `lessOrEqual`, and `notLessOrEqual`,
including their aliases.

They are not available on `matchRegex` and `notMatchRegex`, because matching a
pattern against re-serialized content reintroduces the ordering sensitivity that
`parse` exists to remove. They are also unavailable on assertions that do not
take a `path`, and on the `Raw` assertions, which operate on the whole rendered
document. Using `parse` on an unsupported assertion is an error rather than being
silently ignored.
