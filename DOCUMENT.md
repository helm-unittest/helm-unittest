...
| `matchSnapshot`                       | **path**: *string,optional*. The `set` path for snapshot. <br/>**format**: *string,optional*. Snapshot format: `indexed` (default) or `yaml`. <br/>**matchRegex.pattern**: *string,optional*. The value regex pattern that should exist for snapshot. <br/>**notMatchRegex.pattern**: *string,optional*. The regex pattern that should not exist for snapshot. | Assert the value of **path** is the same as snapshotted last time. <br/>  Assert the value of **matchRegex.pattern** is exist in snapshot. <br/> Assert the value of **notMatchRegex.pattern** is **not  exist** in snapshot. <br/> When **format** is `yaml`, all documents are stored as pure YAML with `---` separators (like `helm template` output) for easier diffs. Check [doc](./README.md#snapshot-testing) below. | <pre>matchSnapshot:<br/>  path: spec<br/>  format: yaml<br/>  matchRegex:<br/>   pattern: .\*a.\*<br/>  notMatchRegex:<br/>   pattern: .\*b.\*<br/></pre>                                                                                                               |
