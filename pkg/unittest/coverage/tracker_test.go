...
func buildTestChart(name string, templates map[string]string) *v2chart.Chart {
	c := &v2chart.Chart{
	...
	for n, body := range templates {
		c.Templates = append(c.Templates, &chartcommon.File{Name: n, Data: []byte(body)})
	}
	return c
}
...
`,
	...
	render := func(values map[string]any) {
		...
		vals, err := chartcommonutil.ToRenderValues(instrumented, values, chartcommon.ReleaseOptions{
			Name:      "rel",
			Namespace: "ns",
			IsInstall: true,
		}, nil)
...
`,
	...
	vals, err := chartcommonutil.ToRenderValues(instrumented, map[string]any{"enabled": true}, chartcommon.ReleaseOptions{
		Name:      "rel",
		Namespace: "ns",
		IsInstall: true,
	}, nil)
...
`,
	...
	vals, err := chartcommonutil.ToRenderValues(instrumented, map[string]any{}, chartcommon.ReleaseOptions{
