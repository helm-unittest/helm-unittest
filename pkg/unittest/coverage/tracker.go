...
type Tracker struct {
	...
	instrumentedChart *v2chart.Chart
...
// modified.
func NewTracker(chart *v2chart.Chart, opts ...TrackerOption) *Tracker {
	t := &Tracker{
...
func (t *Tracker) deepCopyAndInstrument(in *v2chart.Chart, route string, instrument bool) *v2chart.Chart {
	out := new(v2chart.Chart)
	...
	for _, tmpl := range in.Templates {
		copied := &chartcommon.File{Name: tmpl.Name}
	...
	depRoute := func(dep *v2chart.Chart) string {
		return filepath.ToSlash(filepath.Join(route, "charts", dep.Name()))
	}
	deps := make([]*v2chart.Chart, 0, len(in.Dependencies()))
...
// rendering through helm.sh/helm/v4/pkg/engine. It is shared across all jobs and
...
func (t *Tracker) InstrumentedChart() *v2chart.Chart {
